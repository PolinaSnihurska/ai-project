...
const MinimalistChat = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      const res = await fetch('http://localhost:3500/api/chat/message', {
        method: 'POST',
        headers: {
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <button
          ...
          className={`bg-white hover:bg-gray-50 text-gray-800 rounded-full p-1 shadow-lg hover:shadow-xl transition-all duration-300 border border-gray-200 ${isOpen ? 'scale-0' : 'scale-100'}`}
          style={{ width: '70px', height: '70px' }}
        >
          <div className="w-10 h-6 mx-auto relative">
            <img src="/bot_1.png" alt="Bot 1" className="absolute top-0 left-0 w-6 h-6 object-contain animate-bounce" style={{ animationDelay: '0s' }} />
            <img src="/bot_2.png" alt="Bot 2" className="absolute top-0 right-0 w-6 h-6 object-contain animate-bounce" style={{ animationDelay: '0.2s' }} />
          </div>
        </button>
      </div>
      ...
      {isOpen && (
        ...
        >
          ...
          >
            {/* Chat Header */}
            <div className="p-4 border-b border-gray-100 relative" style={{ backgroundColor: '#F7F7F7' }}>
                ...
                {showChatHistory ? (
                  ...
                  <>
                    ...
                    <div className="text-center p-4 flex-1">
                      <h2 className="text-lg font-semibold text-gray-800">Історія чатів</h2>
                    </div>
                    <div className="p-1 w-10"></div> {/* Spacer for alignment */}
                  </>
                ) : (
                  // Regular Chat Header
                  <>
                    <button onClick={() => setIsOpen(false)} className="text-gray-500 hover:text-gray-700 transition-colors p-2">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Chat Messages Area */}
            <div className="flex-1 p-6 overflow-y-scroll" style={{ backgroundColor: '#F7F7F7' }}>
              {showChatHistory ? (
                // Chat History View
                  ...
                  {showWelcome && (
                    <div className="text-center mb-8">
                      <div className="w-42 h-42 mx-auto mt-40 mb-4 bg-none rounded-full flex items-center justify-center">
                        <img src="/say-hi.png" alt="Say Hi" className="w-42 h-42 object-contain" />
                      </div>
                    </div>
                  )}
                        ...
                        {msg.type === 'bot' ? (
                          <div className="flex items-start space-x-3 max-w-[420px]">
                            ...
                            <div className="bg-white rounded-2xl rounded-tl-md px-4 py-2.5 shadow-sm">
                              <div className={`flex items-start space-x-2 ${msg.showButtons ? 'mb-3' : 'mb-0'}`}>
                                <img 
              ...
              {!isVoiceOpen ? (
                <div className="flex items-center space-x-3">
                  <div className="flex-1 relative flex items-center">
                    ...
                    />
                    <button className="p-3 text-gray-500 hover:text-gray-700 transition-colors" onClick={() => setIsVoiceOpen(!isVoiceOpen)}>
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="text-center bg-white rounded-2xl py-10 px-6 shadow-sm">
                  <p className="text-gray-600 mb-8 text-lg font-medium max-w-[220px] mx-auto leading-tight">Ви можете запитати мене все про пристрої</p>
                  <button 
